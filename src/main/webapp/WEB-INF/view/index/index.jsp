	...
	<div class="container-fluid">
		...
		<div class="row">
			<div class="col-md-12"
				...
				<font color="white" size="2px" style="margin-bottom: 5px">下载APP</font>
				
				 <!--注册 -->
				<div style="float: right">
				     <!-- 从session 获取当前有没有登陆  如果登录则不显示注册和登陆按钮-->
				   <c:if test="${sessionScope.user==null }">
				     <button type="button" class="btn btn-link" data-toggle="modal" 
				     data-target="#exampleModal" onclick="reg()"><font color="white" size="2px">注册</font></button>
				     <button type="button" class="btn btn-link" data-toggle="modal" 
				     data-target="#exampleModal" onclick="login()"><font color="white" size="2px">登陆</font></button>
				     </c:if>
				     <c:if test="${sessionScope.user!=null }">
				        <div class="btn-group dropleft">
							<button type="button" class="btn btn-link dropdown-toggle"
								data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">	
								<font color="white" size="2px">登录信息</font></button>
		...
		<div class="row">
			 ...
			 <c:if test="${article.channelId!=null }">
				...
				<div>
				   <c:forEach items="${info.list}" var="article">
					<div class="media">
						<img src="/pic/${article.picture}" style="width: 150px;height: 100px" class="mr-3 rounded" alt="...">
						<div class="media-body">
							<h5 class="mt-0">
							  <a href="/articleDetail?id=${article.id }" target="_blank">${article.title }</a>
							</h5>
							<p style="margin-top: 50px">作者：${article.user.username }·${article.hits }浏览量  ·<fmt:formatDate value="${article.created }"
			...
			<div class="col-md-3">
                 <div class="card" style="wideth:18rem;margin-top:6px">
                    ...
                    <div class="card-body">
                       <c:forEach items="${lastArticles.list }" var="lastArtilce">
                          <div class="media">
                            <img src="/pic/${lastArtilce.picture }" alt="..." class="mr-3 rounded" style="width: 60px;height: 60px">
                             <div class="media-body">
                               <p><a href="articleDetail?id=${lastArtilce.id }" target="_blank">${lastArtilce.title }</a></p>
                             </div>
                          </div>
                          <hr>
                       </c:forEach>
